...
title: Printer with replaceable ink cartridge and print quality IC
...
url: http://patft.uspto.gov/netacgi/nph-Parser?Sect1=PTO2&Sect2=HITOFF&p=1&u=%2Fnetahtml%2FPTO%2Fsearch-adv.htm&r=1&f=G&l=50&d=PALL&S1=07391531&OS=07391531&RS=07391531
...
---
This application is a Continuation of U.S. Ser. No. 11 006 720 filed on Dec. 8 2004 now U.S. Pat. No. 7 206 098 which is a Continuation of U.S. Ser. No. 10 636 265 filed on Aug. 8 2003 now U.S. Pat. No. 7 283 280 which is a Divisional of U.S. Ser. No. 09 436 747 filed on Nov. 9 1999 now issued as U.S. Pat. No. 6 687 022 all of which are herein incorporated by reference.
...
In one broad form the invention comprises a method of printing using two printheads in communication with each other each printhead having a plurality of nozzles each of which discharge ink to produce an ink dot to print the image comprising the steps of

In another form the invention comprises a method of printing using two printheads in communication with each other each printhead having a plurality of nozzles each of which discharge ink to produce an ink dot to print the image comprising the steps of
...
Magazine test is typically typeset in a typeface with serifs such as Times at a point size of 10. At this size an A4 Letter page holds up to 14 000 characters though a typical magazine page holds only about 7 000 characters. Text is seldom typeset at a point size smaller than 5. At 800 dpi text cannot be meaningfully rendered at a point size lower than 2 using a standard typeface. Table 6 illustrates the legibility of various point sizes.
...
For a point size of 9 or greater EDRL slightly outperforms Group 4 simply because Group 4 s runlength codes are turned to 400 dpi.
...
The printhead two printing speeds to allow speed power consumption trade offs to be made in different product configurations.
...
A single pod consists of 10 nozzles sharing a common ink reservoir. 5 nozzles are in one row and are in another. Each nozzle produces dots 22.5 microns in diameter spaced on a 15.875 micron grid. shows the arrangement of a single pod with the nozzles numbered according to the order in which they must be fired.
...
The external ready line indicates that the runlength encoder is ready to encode a runlength and that the current length is available on the runlength lines. It can be used to stall the external logic.
...
The consumer of the data produced by the HCU is the printhead interface. The printhead requires bi level CMYK image data in planar format i.e. with the color planes separated. Further it also requires that even and odd pixels are separated. The output stage of the HCU therefore uses 8 parallel pixel FIFOs one each for even cyan odd cyan even magenta odd magenta even yellow odd yellow even black and odd black.
...
Given a 4 bit halftoned color CMYK and a 1 bit black layer opacity K the composite and clip logic is as defined in Table 22.
