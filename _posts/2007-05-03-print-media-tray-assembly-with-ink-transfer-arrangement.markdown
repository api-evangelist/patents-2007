...
---
This application is a continuation application of U.S. application Ser. No. 09 459 409 filed on Dec. 11 1999 all of which are herein incorporated by reference.

The disclosed invention relates to a print media tray assembly for inclusion in a variety of consumer electronics devices. More particularly the disclosed invention relates to a print media tray assembly featuring a built in print head adapted for OEM inclusion in a variety of consumer electronics devices.
...
According to an aspect of the present disclosure a print media tray assembly comprises a print media tray for storing a stack of print media a housing for receiving therein the print media tray a printhead assembly mounted to the print media tray and a feed mechanism for feeding said print media to the printhead assembly. The print media tray is ejectable from the housing.
...
When a contone object of color Cand opacity is composited with the contone layer the contone layer and the black layer are updated as follows C x y C x y 1 x y if x y 0 Rule 3 x y 0 if x y 0 Rule 4 x y 0 if 0 Rule 5 C x y C x y 1 x y C x y x y Rule 6 
...
The 26.4 MB contone layer consists of 432 538 JPEG blocks giving an overall forward DCT cost of about 80Mcycles. At 150 MHz this equates to about 0.5 seconds which is 25 of the 2 second rendering time allowed per page.
...
The printer driver language monitor and user interface DLL must implement the implement the relevant aspects of the printer control protocol described in Section 6.

The remainder of this section describes the design of the printer driver graphics DLL. It should be read in conjunction with the appropriate Windows 9x NT CE DDK documentation.
