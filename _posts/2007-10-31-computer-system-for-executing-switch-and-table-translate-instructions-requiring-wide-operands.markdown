...
title: Computer system for executing switch and table translate instructions requiring wide operands
...
url: http://patft.uspto.gov/netacgi/nph-Parser?Sect1=PTO2&Sect2=HITOFF&p=1&u=%2Fnetahtml%2FPTO%2Fsearch-adv.htm&r=1&f=G&l=50&d=PALL&S1=08018464&OS=08018464&RS=08018464
...
As shown in an exemplary embodiment of a wide multiply matrix complex doublets instruction W.MUL.MAT.C multiplies memory m15 m14. . . m1 m0 with vector h g f e d c b a yielding products hm14 gm15 . . . bm2 am3 . . . hm12 gm13 . . . bm0 am1hm13 gm12 . . . bm1 am0 
...
As shown in an exemplary embodiment of a wide multiply matrix extract doublets instruction W.MUL.MAT.X.B or W.MUL.MAT.X.L multiplies memory m63 m62 m61 . . . m2 m1 m0 with vector h g f e d c b a yielding the products
 ...
 am7 bm15 cm23 dm31 em39 fm47 gm55 hm63 . . . am2 bm10 cm18 dm26 em34 fm42 gm50 hm58am1 bm9 cm17 dm25 em33 fm41 gm49 hm57am0 bm8 cm16 dm24 em32 fm40 gm48 hm56 rounded and limited as specified.
...
As shown in an exemplary embodiment of a wide multiply matrix floating point half instruction W.MUL.MAT.F multiplies memory m m . . . m m with vector h g f e d c b a yielding products hm gm . . . bm am . . . hm gm . . . bm am0 .

As shown in an exemplary embodiment of a wide multiply matrix complex floating point half instruction W.MUL.MAT.F multiplies memory m m . . . m m with vector h g f e d c b a yielding products hm gm . . . bm am3 . . . hm gm . . . bm am hm gm . . . bm am0 .
...
As shown in as an example with specific register values a wide convolve extract doublets instruction W.CONVOLVE.X.B or W.CONVOLVE.X.L with start in rb 24 convolves memory vector rc c c . . . c c with memory vector rd d15 d . . . d d yielding the products cd cd . . . cd cd0 cd cd . . . cd cd . . . cd cd . . . cd cd0 cd cd . . . cd cd rounded and limited as specified by the contents of general register rb. The values c . . . c are not used in the computation and may be any value.

As shown in as an example with specific register values a wide convolve extract doublets instruction W.CONVOLVE.X.L with mpos in rb 8 and mzero in rb 48 so length 512 mzero dmsize 512 13 convolves memory vector rc c c . . . c c with memory vector rd d d . . . d d yielding the products cd cd . . . cd cdcd cd . . . cd cd . . . cd cd . . . cd cdcd cd . . . cd cd rounded and limited as specified. In this case the starting position is located so that the useful range of values wraps around below c to c31 . . . 28. The values c . . . c are not used in the computation and may be any value. The length parameter is set to 13 so values of d . . . d must be zero.

In this case the starting position is located so that the useful range of values wraps around below c to c31 . . . 25. The length parameter is set to 13 so values of d . . . d are expected to be zero.

As shown in as an example with specific register values a wide convolve extract doublets two dimensional instruction W.CONVOLVE.X.B or W.CONVOLVE.X.L with mpos in rb 24 and vsize in rc and rd 4 convolves memory vector rc c c . . . c c . . . c c with memory vector rd d d . . . d d . . . d1 d yielding the products cd cd . . . cd cd . . . cd cd0 cd cd . . . cd cd . . . cd cd . . . cd cd . . . cd cd . . . cd cd cd cd . . . cd cd . . . cd cd rounded and limited as specified by the contents of general register rb.

As shown in as an example with specific register values a wide convolve extract complex doublets instruction W.CONVOLVE.X.B or W.CONVOLVE.X.L with n set in rb with mpos in rb 12 convolves memory vector rc c c . . . c c with memory vector rd d d . . . d d yielding the products cd cd . . . cd cdcd cd . . . cd cdcd cd . . . cd cd0 cd cd . . . cd cd rounded and limited as specified by the contents of general register rb.
...
When replacing set vsc for a LineStream or SubStream replacement the priority for replacement is unchanged unless another set contains the invalid MESI state computing a new vsc by vsc 
...
However the orderings are simplified in the first Zeus implementation to reduce the number of vs bits to one per set keeping a two bit vsc state value 3 2 1 0 
...
The tags for these entries are indexed by global virtual address . . . and contain access control information detailed below.
