...
In the initial view the Active Path Active Path comprises a single active link termed a root link . Since the initial view includes only one active link it is both the root link and the end link .
